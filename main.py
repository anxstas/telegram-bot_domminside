...
🤿 Пойти глубже — там есть важное и полезное для тебя, а ещё — детальнее о подходе. Ну и, конечно, там "SOS без соплей", куда можно заходить только в крайних эмоциях и очень осторожно.
...
@bot.message_handler(func=lambda msg: msg.text == '❤️ Хочу тепла')
...
@bot.message_handler(func=lambda msg: msg.text == "Да пошло всё нахуй")
...
@bot.message_handler(func=lambda msg: msg.text == "Я ща взорвусь к хуям")
...
@bot.message_handler(func=lambda msg: msg.text == "Уже просто за край, нахуй")
...
@bot.message_handler(func=lambda msg: msg.text == "Что это за нахуй?")
...
@bot.message_handler(func=lambda msg: msg.text == "Заебался быть сильным")
...
@bot.message_handler(func=lambda msg: msg.text == "Не знаю зачем жму, но я жму")
...
@bot.message_handler(func=lambda msg: msg.text == "Просто хочу полежать")
...
def anchor_support(message):
    text = (
        "Иногда единственное, что можно — держаться. За дыхание. За плед. За взгляд кошки. За звук своего имени.\n\n"
        "🔸 Положи руку на грудь и почувствуй, что ты дышишь. Это — жизнь.\n"
        "🔸 Посмотри по сторонам и найди 3 предмета, которые дают тебе чувство опоры.\n"
        "🔸 Напиши себе на бумаге: \"Я здесь. Я держусь.\"\n\n"
...
def reassurance_support(message):
    messages = [
        "Ты не сломан. Ты — живой. А живым бывает очень больно.",
        "Ты не должен быть сильным всё время. Иногда сильный — это тот, кто просто продолжает жить.",
...
def collapse_support(message):
    text = (
        ...
        "Ты имеешь право остановиться. Лечь. Смотреть в потолок. Просто быть.\n\n"
        "🔸 Представь, что ты — под пледом, в домике, где никто не тронет. Можешь там остаться.\n"
        "🔸 Дыши медленно: вдох — 4 счета, выдох — 6. Повтори 5 раз.\n\n"
...
def fear_of_forever(message):
    text = (
        ...
        "🔸 Даже мысль 'это не навсегда' — уже шаг в сторону надежды.\n\n"
        "Я с тобой. Подышим?"
    )
    bot.send_message(message.chat.id, text)
...
def handle_deeper(message):
    ...
    markup.add("🥟 Какой ты пельмень?", "🐳 Еще глубже")
...
@bot.message_handler(func=lambda msg: msg.text == "🥟 Какой ты пельмень?")
...
def interpret_dumpling_result(chat_id, uid):
    ...
    if result == 'classic':
        ...
        desc = (
            "🔥 Ты — Пельмень-Фронтмен.\n\n"
            "Ты — огонь. Когда ты входишь в сковородку, вся кухня знает.\n"
            "Ты флексишь, исчезаешь, обжигаешь — но под хрустом скрыта мягкость.\n"
            "Ты драматичен, блистаешь и умеешь делать эффектные выходы.\n\n"
            "✨ Аффирмация: Я не для всех — и это мой соус. Я обжигаю, потому что живу на полной температуре."
        )
    else:
        desc = (
            "🌱 Ты — Пельмень-Философ.\n\n"
            "Ты — загадка в морозилке. Странный, глубокий, не для всех.\n"
...
def handle_ultimate_answer(message):
    ...
    bot.send_message(
        message.chat.id,
        "Тогда держи 🐾\n\nОТВЕТ: 42\n\nЭтого ответа с нетерпением веками ждали все разумные расы, ведь он должен был решить все проблемы Вселенной. Теперь все в твоих руках. Соберись 💛",
        reply_markup=markup
    )
...
def gpt_flow(message):
    ...
    if step == 1:
        ...
        markup = types.ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
        markup.add("❤️ Хочу тепла", "🧘 Техники", "🧸 Поддержи меня", "🏠 Домой")
