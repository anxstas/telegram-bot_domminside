...
bai_questions = [
    ...
    "Страх, что случится самое худшее", "Головокружение или ощущение дурноты", "Учащённое сердцебиение",
    "Неустойчивость", "Чувство ужаса", "Ощущение удушья", "Дрожь", "Чувство удушья", "Расстройства желудка",
    "Головная боль", "Чувство напряженности", "Ощущение нереальности происходящего", "Страх утраты контроля",
    "Затруднённое дыхание", "Страх смерти", "Потливость", "Ощущение удушья", "Тошнота или боль в животе"
]

stai_questions = [
    "Я чувствую себя спокойным(ой)", "Я испытываю напряжение", "Я обеспокоен(а) возможными неприятностями",
    "Я чувствую себя свободно", "Я нервничаю", "Я чувствую себя обеспокоенным(ой)",
    "Я чувствую себя уверенно", "Я раздражителен(льна)", "Я чувствую себя подавленным(ой)",
    "Я испытываю страх", "Я легко устаю", "Я чувствую себя скованным(ой)"
]

bai_levels = [(0, 7, "Минимальная тревожность"), (8, 15, "Легкая"), (16, 25, "Умеренная"), (26, 63, "Тяжелая")]
stai_levels = [(0, 20, "Низкая тревожность"), (21, 40, "Средняя тревожность"), (41, 60, "Высокая тревожность")]

bai_descriptions = {
    "Минимальная тревожность": "Ваш уровень тревоги невысокий и не сопровождается значительными телесными симптомами. Это признак устойчивости.",
    "Легкая": "Вы можете замечать телесные проявления тревоги: напряжение, потливость, головные боли. Это стоит учитывать в повседневной жизни.",
    "Умеренная": "Симптомы тревоги становятся регулярными и могут вызывать дискомфорт. Возможно, стоит обсудить это с психологом.",
    "Тяжелая": "Телесные и эмоциональные симптомы тревоги явно выражены. Это может быть связано с генерализованной тревогой или паническими состояниями."
}

stai_descriptions = {
    "Низкая тревожность": "Вы демонстрируете устойчивость как в ситуациях стресса, так и в долгосрочной перспективе. Это говорит о здоровом уровне эмоционального регулирования.",
    "Средняя тревожность": "Вы испытываете тревогу в отдельных ситуациях и можете быть склонны к внутреннему напряжению. Это естественно, но важно наблюдать за частотой.",
    "Высокая тревожность": "Вы склонны к частому внутреннему напряжению и воспринимаете многие ситуации как стрессовые. Это может указывать на ситуативную или личностную тревожность."
}
...
def send_bai_question(chat_id, uid):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=False)
    markup.add("0 — Совсем нет", "1 — Немного", "2 — Умеренно", "3 — Сильно")
...
def send_stai_question(chat_id, uid):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=False)
    markup.add("0 — Почти никогда", "1 — Иногда", "2 — Часто", "3 — Почти всегда")
