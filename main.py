...
🟡 Сессия-знакомство со скидкой 40% (30€ вместо 50€).
...
def reassurance_support(message):
    messages = [
        "С тобой всё нормально. Я знаю, что иногда это вызывает сомнения, но тебя просто так научили. А на самом деле, с тобой всё ок.\n\n"
...
@bot.message_handler(func=lambda msg: msg.text == "🛁 Тест глубины")
...
dumpling_questions = [
    {
        'q': """Благодаря прохождению Теста глубины "🥟 Какой ты пельмень?" ты сможешь прикоснуться к невиданным доселе глубинам собственной личности, более тонко отстроить свой пельменный архетип, что позволит тебе перейти на следующий круг своего личного ада гораздо раньше своих недалеких родственников.
...
def handle_ultimate_answer(message):
    ...
    bot.send_message(
        message.chat.id,
        "Тогда держи 🐾\n\nОТВЕТ: 42\n\nЭтого ответа с нетерпением веками ждали все разумные расы, ведь он должен был решить все проблемы Вселенной. Теперь все в твоих руках. Соберись, тряпка 💛",
        reply_markup=markup
    )
