...
# --- Состояние пользователя GAD-7 ---
user_gad7_state = {}

gad7_questions = [
    "Чувствовали ли вы себя нервным, тревожным или напряжённым за последние 2 недели?",
    "С трудом останавливали или контролировали чувство тревоги?",
    "Чрезмерно беспокоились о разных вещах?",
    "Сложно было расслабиться?",
    "Беспокойство было настолько сильным, что вы не могли усидеть на месте?",
    "Часто раздражались или были вспыльчивы?",
    "Боялись, что случится что-то ужасное?"
]
...
gad7_descriptions = {
    "Минимальная тревожность": "Ваш уровень тревоги низкий и не вызывает клинических опасений. Можно проконсультироваться о способах заботы о себе.",
    "Легкая тревожность": "Вы испытываете легкую тревожность, которая может усиливаться. Важно прислушаться к себе. Понаблюдайте. Можно проконсультироваться о способах заботы о себе.",
    "Умеренная тревожность": "Проявляются регулярные симптомы тревоги - напряжение, беспокойные мысли, возможно, вопросы сна и проявленности тревоги в теле. Важно прислушаться к себе. Не игнорируйте.",
    "Тяжелая тревожность": "Тревога значительно влияет на качество жизни. Важно заняться собой сейчас."
}
...
def send_gad7_question(chat_id):
    step = user_gad7_state[chat_id]["step"]
    if step >= len(gad7_questions):
        show_gad7_result(chat_id)
        return
    q = gad7_questions[step]
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=False)
    markup.add("0 — Совсем нет", "1 — Немного")
    markup.add("2 — Заметно", "3 — Почти каждый день")
    bot.send_message(chat_id, f"{step+1}. {q}", reply_markup=markup)

@bot.message_handler(func=lambda m: m.chat.id in user_gad7_state and m.text[0] in "0123")
def handle_gad7_answer(message):
    chat_id = message.chat.id
    user_gad7_state[chat_id]["answers"].append(int(message.text[0]))
    user_gad7_state[chat_id]["step"] += 1
    send_gad7_question(chat_id)
    ...
    for minv, maxv, level in gad7_levels:
        if minv <= total <= maxv:
            ...
            bot.send_message(
                ...
                f"{desc}\n\n"
                "Можете отправить скрин Стасу лично на @anxstas — и он аккуратно подскажет, что можно с этим сделать. Это бесплатно."
            )
            break
...
phq9_questions = [
    "Мало интереса или удовольствия от обычных дел?",
    "Печаль, подавленность, безнадёжность?",
    "Бессонница или чрезмерная сонливость?",
    "Усталость, нехватка энергии?",
    "Проблемы с аппетитом?",
    "Плохое мнение о себе, чувство вины?",
    "Трудности с концентрацией?",
    "Замедленность или повышенная активность?",
    "Мысли о саморазрушении или смерти?"
]
...
phq9_descriptions = {
    "Минимальная депрессия": "Ваше настроение в пределах нормы. И это здорово. Можно проконсультироваться о способах заботы о себе.",
    "Легкая депрессия": "Присутствуют первые признаки подавленности. Важно прислушаться к себе. Понаблюдайте. Можно проконсультироваться о способах заботы о себе.",
    "Умеренная депрессия": "Симптомы депрессии выражены. Не игнорируйте. Обратитесь к психотерапевту. Обсудите, что у вас происходит в жизни, и где вы живете не так, как хотите.",
    "Тяжелая депрессия": "Выраженная депрессия с серьёзным влиянием на повседневную жизнь. Вам важно заняться собой сейчас."
}
...
def send_phq9_question(chat_id):
    ...
    markup.add("2 — Часто", "3 — Почти каждый день")
    bot.send_message(chat_id, f"{step+1}. {q}", reply_markup=markup)

@bot.message_handler(func=lambda m: m.chat.id in user_phq9_state and m.text[0] in "0123")
def handle_phq9_answer(message):
    chat_id = message.chat.id
    user_phq9_state[chat_id]["answers"].append(int(message.text[0]))
    user_phq9_state[chat_id]["step"] += 1
    send_phq9_question(chat_id)

def show_phq9_result(chat_id):
    uid = chat_id
    total = sum(user_phq9_state[uid]['answers'])
    for minv, maxv, level in phq9_levels:
        if minv <= total <= maxv:
            desc = phq9_descriptions[level]
            bot.send_message(
                chat_id,
...
мой канал в телеграм (о тревоге) и инста, где идет "Тоска'на". А еще - та самая песня, написанная в период ГТР и ТДР, через которую я в течение года проживал и принимал смерть папы.""",
...
def handle_emergency_reply(message):
    ...
    uid = message.from_user.id
    human_delay()
    bot.send_message(uid, "Спасибо, что делишься. Хоть я и ИИ, но я тебя слышу. Твой вопрос важный, как и всё, что происходит с тобой... И я с тобой в этом, насколько могу.")

    human_delay()
    bot.send_message(uid, "Хочешь, перешли свое сообщение прямо сейчас Стасу лично на @anxstas? 🙂 Он ответит, как только прочитает. Просто скопируй и отправь (или отправь скриншот), без приветствий, я его предупрежу. Это бесплатно.")
...
stai_state_questions = [
    "Я чувствую себя спокойно", "Я напряжён(а)", "Я испытываю внутреннее спокойствие", "Я чувствую себя уравновешенно",
    "Я чувствую себя встревоженным(ой)", "Я чувствую себя комфортно", "Я чувствую себя обеспокоенным(ой)", "Я чувствую себя уверенно",
    "Я нервничаю", "Я спокоен(на)", "Я испытываю раздражение", "Я чувствую себя довольным(ой)", "Я чувствую напряжение",
    "Я чувствую себя расслабленным(ой)", "Я нахожусь в состоянии волнения", "Я чувствую себя беспокойным(ой)",
    "Я чувствую тревогу", "Я не испытываю напряжения", "Я чувствую страх", "Я чувствую себя спокойно"
]

stai_trait_questions = [
    "Я часто чувствую тревогу", "Я стараюсь избегать критических ситуаций", "Я легко расстраиваюсь", "Я чувствую, что часто волнуюсь",
    "Я способен(на) сохранять спокойствие в трудных ситуациях", "Меня часто беспокоят мелочи", "Я быстро раздражаюсь",
    "Я часто испытываю внутреннее напряжение", "Я эмоционально стабилен(на)", "Меня легко выбить из равновесия",
    "Я склонен(на) тревожиться по пустякам", "Я чувствую себя спокойно в стрессовых ситуациях", "Я часто чувствую неуверенность",
    "Я переживаю заранее о возможных проблемах", "Я реагирую на стресс остро", "Я обычно уравновешен(на)",
    "Я склонен(на) к длительным переживаниям", "Я умею держать себя в руках", "Я чувствую, что тревожность — часть моей натуры",
    "Я быстро успокаиваюсь после стресса"
]

stai_levels = [
    (20, 39, "Низкий уровень тревожности"),
    (40, 59, "Средний уровень тревожности"),
    (60, 80, "Высокий уровень тревожности")
]

stai_state_interpretation = {
    "Низкий уровень тревожности": "Ваше текущее состояние характеризуется относительным спокойствием и отсутствием острого напряжения. Можно проконсультироваться о способах заботы о себе.",
    "Средний уровень тревожности": "Вы в данный момент испытываете заметное эмоциональное напряжение, внутреннюю активность или беспокойство. Важно прислушаться к себе. Не игнорируйте. Обратитесь за помощью.",
    "Высокий уровень тревожности": "Текущее состояние указывает на значительное эмоциональное напряжение. Возможно, вы находитесь в состоянии тревожного возбуждения или паники. Важно заняться собой сейчас."
}
stai_trait_interpretation = {
    "Низкий уровень тревожности": "Вы описываете себя как достаточно уравновешенного человека. Ситуации вызывают тревогу редко. Можно проконсультироваться о способах заботы о себе.",
    "Средний уровень тревожности": "Вам часто приходится сталкиваться с внутренней тревогой, вы склонны к чрезмерному беспокойству. Это может отражаться на поведении и качестве жизни. Важно прислушаться к себе. Не игнорируйте. Обратитесь за помощью.",
...
@bot.message_handler(func=lambda m: m.chat.id in user_bai_state and m.text and m.text.strip()[0] in "0123")
...
def show_bai_result(chat_id):
    ...
    for minv, maxv, level in [
        ...
        if minv <= total <= maxv:
            ...
            break  # нашли интерпретацию — выходим

    # Показ доп. клавиатуры
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=False)
    markup.add("🟡 Записаться на сессию-знакомство -40%")
    markup.add("🏠 Домой")
    bot.send_message(chat_id, "🟡 Также это можно обсудить глубже — запишитесь на сессию 👇", reply_markup=markup)
...
# --- STAI (Spielberger) тест: состояние и черта ---
user_stai_state = {}
...
def send_stai_question(chat_id, uid):
    ...
    questions = user_stai_state[uid]["questions"]
    ...
    bot.send_message(chat_id, f"{idx+1}. {q}", reply_markup=markup)

@bot.message_handler(func=lambda m: m.chat.id in user_stai_state and m.text and m.text.strip()[0] in "0123")
def handle_stai_answer(message):
    uid = message.chat.id
    user_stai_state[uid]["answers"].append(int(message.text.strip()[0]))
    user_stai_state[uid]["index"] += 1
    send_stai_question(message.chat.id, uid)

def show_stai_result(chat_id, uid):
    answers = user_stai_state[uid]["answers"]
    state_score = sum(answers[:20])
    trait_score = sum(answers[20:])

    def interpret(score, levels, descriptions):
        for minv, maxv, label in levels:
            if minv <= score <= maxv:
                return label, descriptions[label]
        return "Не определено", "Не удалось интерпретировать результат."

    state_level, state_text = interpret(state_score, stai_levels, stai_state_interpretation)
    trait_level, trait_text = interpret(trait_score, stai_levels, stai_trait_interpretation)

    result_message = (
        f"🧠 *Ваш результат по STAI:*\n\n"
        f"*Текущее состояние тревожности* — {state_score}/80\n"
        f"_{state_level}_\n{state_text}\n\n"
        f"*Личностная тревожность* — {trait_score}/80\n"
        f"_{trait_level}_\n{trait_text}\n\n"
        "Можете отправить скрин Стасу лично на @anxstas — и он аккуратно подскажет, что можно с этим сделать. Это бесплатно."
    )
...
cesd_questions = [
    ...
    "Вы чувствовали, что не хватает сил на повседневные дела?", "За последнюю неделю вам казалось, что всё требует чрезмерных усилий?",
    "Вы чувствовали себя одиноким(ой)?", "За последнюю неделю вам было трудно сосредоточиться на том, что вы делаете?",
    "Вас не покидала грусть большую часть дня?", "За последнюю неделю вы думали, что всё, что вы делаете — бесполезно?",
    "Вы теряли интерес к любимым занятиям?", "За последнюю неделю вам казалось, что люди вас не понимают?",
    "Вам не хватало энергии?", "За последнюю неделю ничто не приносило радости?", "За последнюю неделю вы чувствовали, что вас никто не любит?",
    "Вам казалось, что вас никто не поддерживает?", "За последнюю неделю вы спали больше или меньше обычного?",
    "Вы питались хуже обычного или переедали?", "За последнюю неделю вы ощущали, что жизнь потеряла смысл?",
    "Вы избегали общения с людьми?", "За последнюю неделю вы были тревожны или раздражительны без причины?",
    "Вы чувствовали, что ничего не стоите?"
]
