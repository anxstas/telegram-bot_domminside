...
🤿 Пойти глубже — там есть важное и полезное для тебя, а ещё — детальнее о подходе.
...
def sos_burnout(message):
    bot.send_message(
        message.chat.id,
        "Ты имеешь право на усталость. Мир требует многого. Иногда — невозможного. "
        "Сейчас, возможно, лучшее, что ты можешь сделать — просто отстать от себя. И выдохнуть."
    )

@bot.message_handler(func=lambda msg: msg.text == "💣 Когда хочется разъебать")
def sos_rage(message):
    bot.send_message(
        message.chat.id,
        "Злость — это энергия. И часто — крик о том, что твоё нарушено. "
        "Ты имеешь право на ярость. Главное — не забыть, что ты в ней не один. Дыши."
    )

@bot.message_handler(func=lambda msg: msg.text == "😵‍💫 Когда в ахуе")
def sos_confused(message):
    bot.send_message(
        message.chat.id,
        "Кажется, сейчас всё перемешано. Это нормально. Иногда ахуй — это защита. "
        "Просто останься на месте. Ничего не решай. Ничего не меняй. Побудь рядом с собой."
    )

@bot.message_handler(func=lambda msg: msg.text == "💀 Хочу сдохнуть, но не умирать")
def sos_suicidal(message):
    bot.send_message(
        message.chat.id,
        "Очень важно, что ты здесь и читаешь это. Это уже шаг. "
        "Если хочется исчезнуть — это, возможно, просто крик души о передышке. "
        "Пожалуйста, останься. Даже если всё внутри просит уйти — ты нужен. Ты важен. Правда."
    )

@bot.message_handler(func=lambda msg: msg.text == "😐 Не хочется ни хуя, но нажал")
def sos_apathy(message):
    bot.send_message(
        message.chat.id,
        "Ты всё равно нажал. А значит — где-то в тебе ещё теплится жизнь. "
        "Даже если кажется, что ты не чувствуешь — это уже чувство. "
        "Просто побудь. Ничего не делай. Просто побудь здесь со мной."
    )

@bot.message_handler(func=lambda msg: msg.text == "🐾 Назад в помощь. Ну или Бог в помощь")
def sos_back_to_help(message):
    # Здесь ты можешь вернуть человека в раздел "Срочная помощь"
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add("🆘 Срочная помощь", "🏠 Главное меню")
    bot.send_message(message.chat.id, "Возвращаю в помощь. Ты не один 💛", reply_markup=markup)
